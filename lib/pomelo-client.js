(function() {
  ...
  var defaultEncode = pomelo.encode = function(reqId, route, msg) {
    ...
    var droute = route.replace(/\./g, '_');
    if(protobuf && clientProtos[route]) {
      msg = protobuf.encode(route, msg);
    } else if(decodeIO_encoder && decodeIO_encoder.lookup(droute)) {
      var Builder = decodeIO_encoder.build(droute);
  ...
  var connect = function(params, url, cb) {
    ...
    if(window.localStorage && window.localStorage.getItem('protos') && protoVersion === 0) {
      ...
      if(!!decodeIO_protobuf) {
        decodeIO_encoder = decodeIO_protobuf.loadProto(clientProtos);
        decodeIO_decoder = decodeIO_protobuf.loadProto(serverProtos);
      }
    }
    //Set protoversion
  ...
  var deCompose = function(msg) {
    ...
    } else if(decodeIO_decoder && decodeIO_decoder.lookup(droute)) {
        if (msg.body[0] == MAGIC_NUMBER) {
          var tmp = new Uint8Array(msg.body.length - 1);
          for (var i = 0; i < tmp.length; i++) {
            tmp[i] = msg.body[i + 1];
          }
          msg.body = tmp;
          return decodeIO_decoder.build(droute).decode(msg.body);
        }
        else {
